...
@interface DHSemgentView()<UIScrollViewDelegate>{
...
// 所有标题的设置
@property (strong, nonatomic) DHSegmentStyle *segmentStyle;
@property (copy, nonatomic) ExtraBtnOnClick extraBtnOnClick;
@property (weak, nonatomic) id<DHScrollPageDelegate> delegate;
...
- (instancetype)initWithFrame:(CGRect )frame segmentStyle:(DHSegmentStyle *)segmentStyle delegate:(id<DHScrollPageDelegate>)delegate titles:(NSArray *)titles pictures:(NSArray *)pictures select_pictures:(NSArray *)select_pictures titleDidClick:(TitleBtnOnClickBlock)titleDidClick
{
    if (self = [super initWithFrame:frame]) {
        ...
        self.select_pictures = select_pictures;
...
- (void)titleLabelOnClick:(UITapGestureRecognizer *)tapGes {
    ...
    DHTitleView *currentLabel = (DHTitleView *)tapGes.view;
...
- (void)setupTitles {
    ...
    for (NSString *title in self.titles) {
        ...
        DHTitleView *titleView = [[DHTitleView alloc] initWithFrame:CGRectZero];
        titleView.tag = index;
        ...
        titleView.normalImage = self.pictures[index];
        titleView.selectedImage = self.select_pictures[index];
    ...
    ((DHTitleView*)self.titleViews[0]).selected = YES;
}
...
- (void)setupUI {
    ...
    if (self.segmentStyle.isScrollTitle) { // 设置滚动区域
        DHTitleView *lastTitleView = (DHTitleView *)self.titleViews.lastObject;
...
- (void)setUpTitleViewsPosition {
    ...
    CGFloat titleY = 0.0;
    CGFloat titleW = 0.0;
    CGFloat titleH = self.zj_height - self.segmentStyle.scrollLineHeight-3;
    
    if (!self.segmentStyle.isScrollTitle) {// 标题不能滚动, 平分宽度
        titleW = self.scrollView.bounds.size.width / self.titles.count;
        
        NSInteger index = 0;
        for (DHTitleView *titleView in self.titleViews) {
    ...
    DHTitleView *currentTitleView = (DHTitleView *)self.titleViews[_currentIndex];
    ...
    if (currentTitleView) {
        ...
        if (self.segmentStyle.isShowImage) {
            ...
            [currentTitleView setSelected:YES];
...
- (void)setupScrollLineAndCover {
    ...
    DHTitleView *firstLabel = (DHTitleView *)self.titleViews[0];
...
- (void)adjustUIWhenBtnOnClickWithAnimate:(BOOL)animated taped:(BOOL)taped {
    ...
    DHTitleView *oldTitleView = (DHTitleView *)self.titleViews[_oldIndex];
    DHTitleView *currentTitleView = (DHTitleView *)self.titleViews[_currentIndex];
    
    CGFloat animatedTime = animated ? 0.0 : 0.0;
...
- (void)adjustUIWithProgress:(CGFloat)progress oldIndex:(NSInteger)oldIndex currentIndex:(NSInteger)currentIndex {
    ...
    DHTitleView *oldTitleView = (DHTitleView *)self.titleViews[oldIndex];
    DHTitleView *currentTitleView = (DHTitleView *)self.titleViews[currentIndex];
...
- (void)adjustTitleOffSetToCurrentIndex:(NSInteger)currentIndex {
    ...
    int index = 0;
    for (DHTitleView *titleView in _titleViews) {
    ...
    if (self.scrollView.contentSize.width != self.scrollView.bounds.size.width + contentSizeXOff) {// 需要滚动
        DHTitleView *currentTitleView = (DHTitleView *)_titleViews[currentIndex];
...
- (UIButton *)extraBtn {
    ...
    if (!_extraBtn) {
        ...
        NSString *imageName = self.segmentStyle.extraBtnBackgroundImageName ? self.segmentStyle.extraBtnBackgroundImageName : @"BKPhone_contact";
